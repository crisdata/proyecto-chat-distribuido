...
import { listarUsuarios, buscarGrupos, crearGrupo, unirseAGrupo, gruposMios } from "../services/api";
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
			...
			>
				...
				<div className="flex gap-1 px-3 pt-3 pb-1">
					<TabButton tab="personas" actual={tab} onClick={cambiarTab} icon={User}>
						Personas
					</TabButton>
					<TabButton tab="grupos" actual={tab} onClick={cambiarTab} icon={Users}>
						Grupos
					</TabButton>
					<TabButton tab="crear" actual={tab} onClick={cambiarTab} icon={PlusCircle}>
						Crear
					</TabButton>
				</div>
				...
				{tab !== "crear" && (
					<div className="px-5 pt-2 pb-2">
						<div className="flex items-center gap-2 bg-vibe-800 rounded-xl
							...
							<input
								type="text"
								placeholder={tab === "personas" ? "Buscar usuario..." : "Buscar grupo..."}
								value={busqueda}
			...
			return (
				<div className="flex flex-col items-center justify-center py-8 gap-2">
					...
					<p className="text-vibe-500 text-sm">
						{busqueda ? "No se encontraron usuarios" : "No hay usuarios nuevos disponibles"}
					</p>
				</div>
			);
		}
	...
	function renderGrupos() {
		return gruposBusqueda.map((grupo) => (
			<button key={grupo.id}
				...
				<div className="flex-1 min-w-0">
					<p className="text-sm font-medium text-vibe-200 truncate">{grupo.nombre}</p>
					<p className="text-xs text-vibe-500">
		...
		return (
			<div className="flex flex-col gap-3 px-3 py-2">
				<label className="flex flex-col gap-2">
					<span className="text-xs font-medium text-vibe-400">Nombre del grupo</span>
					<input
						type="text"
